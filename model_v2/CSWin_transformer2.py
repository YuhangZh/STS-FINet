...
class CSWinBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.drop_path(attened_x.add(x_des))
...
class CSWinTransformer(nn.Module):
    ...
    def __init__(self, img_size=512, patch_size=16, in_chans=3, num_classes=1000, embed_dim=96, depth=[2, 2, 6, 2],
                 split_size=[3, 5, 7],
...
def CSWin_De(img_size=128, patch_size=4, in_chans=384, embed_dim=144, depth=2, **kwargs):
    model = CSWinTransformer(img_size=img_size, patch_size=patch_size, in_chans=in_chans,
        embed_dim=embed_dim, depth=depth, split_size=1, num_heads=4, mlp_ratio=4., **kwargs)
    model.default_cfg = default_cfgs['cswin_384']
    return model
