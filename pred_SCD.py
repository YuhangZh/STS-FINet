...
#################################
from dataset_process import Landsat_SCD as RS
# from dataset_process import SECOND_process as RS
from model_v2.network import MCDNet as Net
DATA_NAME = 'Landsat'
# DATA_NAME = 'SECOND'
#################################
args = {'gpu_id': '0,1'}    # Landsat-SCD
# args = {'gpu_id': '0'}  # SECOND
...
def main():
    ...
    net = Net(num_classes=RS.num_classes, img_size=RS.size).to(device)
...
def predict(net, pred_set, pred_loader, pred_dir):
    ...
    for vi, data in enumerate(pred_loader):
        ...
        for i in range(inter, inter + imgs_A.shape[0]):
            ...
            mask_name = pred_set.get_mask_name(i)
